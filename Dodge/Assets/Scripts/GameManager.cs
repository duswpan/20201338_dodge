...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bulletSpawners[0].x = -4f;
        bulletSpawners[0].y = 1f;
        bulletSpawners[0].z = 4f;

        bulletSpawners[1].x = 4f;
        bulletSpawners[1].y = 1f;
        bulletSpawners[1].z = 4f;

        bulletSpawners[2].x = 4f;
        bulletSpawners[2].y = 1f;
        bulletSpawners[2].z = -4f;

        bulletSpawners[3].x = -4f;
        bulletSpawners[3].y = 1f;
        bulletSpawners[3].z = -4f;
    }
    ...
    void Update()
    {
        if (!isGameover)
        {
            ...
            if (surviveTime % 5f <= 0.01f && prevItemCheak == 4)
            {
                Vector3 randpos = new Vector3(Random.Range(-4f, 4f), 0f, Random.Range(-4f, 4f));
            ...
            prevItemCheak = (int)(surviveTime % 5f);
            ...
            if (surviveTime < 5f && spawnCounter == 0)
            {
                ...
                level.GetComponent<Rotator>().ratationSpeed += 15f;
                spawnCounter++;
            }
